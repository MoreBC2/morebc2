...
| Local BitcoinII node | Locally tested | BitcoinII Core `v29.1.0` GUI inspection and a separate wallet-disabled CLI route were tested on Windows mainnet. A later isolated test reproduced default DNS bootstrap twice without borrowed peers and tested both configured seed hostnames independently. See the [Windows operator test](windows-node-operator-test-2026-08-27.md) and [fresh-node peer discovery test](windows-peer-discovery-test-2026-08-28.md). | Full sync, cross-environment seed reliability, fixed-seed recovery, other platforms, wallet behavior, and universal port behavior remain unresolved. |
